...
cursor += 32;
console.log("Previous hash:", Utils.HEX.encode(prevHash));

// Merkle root (32 bytes)
const merkleRoot = buffer.slice(cursor, cursor + 32);
cursor += 32;
console.log("Merkle root:", Utils.HEX.encode(merkleRoot));
...
console.log("First 20 bytes of transaction:", Utils.HEX.encode(txHeader));
...
cursor += 32;
console.log("Previous tx hash:", Utils.HEX.encode(prevTxHash));
...
if (cursor + scriptLen > buffer.length) {
    ...
    console.log("Script bytes:", Utils.HEX.encode(script));
    cursor += scriptLen;
}
    ...
    for (let i = 0; i < Math.min(outputCount, 1); i++) {
        ...
        if (cursor < buffer.length) {
            ...
            if (cursor + tokenLen <= buffer.length) {
                ...
                cursor += tokenLen;
                console.log("Token ID:", Utils.HEX.encode(tokenId));
            }
