import { TestParams } from "./src/net/bigtangle/params/TestParams";
import { Buffer } from "buffer";

const tip = "";

const PARAMS = TestParams.get();
const serializer = PARAMS.getSerializer(true);
const blockde = serializer.makeBlock(Buffer.from(tip, 'hex'));

console.log("Original hex length:", tip.length);
console.log("Original buffer length:", Buffer.from(tip, 'hex').length);

const blockbyte = blockde.bitcoinSerializeCopy();
console.log("Reserialized buffer length:", blockbyte.length);

console.log("Original hex:", tip);
console.log("Reserialized hex:", blockbyte.toString('hex'));

// Compare byte by byte
const original = Buffer.from(tip, 'hex');
const reserialized = blockbyte;

console.log("\nByte comparison:");
for (let i = 0; i < Math.min(original.length, reserialized.length); i++) {
    if (original[i] !== reserialized[i]) {
        console.log(`Difference at byte ${i}:`);
        console.log(`  Original: 0x${original[i].toString(16).padStart(2, '0')}`);
        console.log(`  Reserialized: 0x${reserialized[i].toString(16).padStart(2, '0')}`);
        break;
    }
}

if (original.length !== reserialized.length) {
    console.log(`Length mismatch: ${original.length} vs ${reserialized.length}`);
}
