function debugTransaction1() {
  const tip =
    "";

  // Convert hex string to array of bytes
  const bytes = [];
  for (let i = 0; i < tip.length; i += 2) {
    bytes.push(parseInt(tip.substr(i, 2), 16));
  }
  
  console.log(`Total bytes: ${bytes.length}`);
  
  // Parse transaction 1 starting at position 163
  console.log(`Parsing transaction 1 at position 163:`);
  let cursor = 163;
  
  // Version (4 bytes)
  const version = (bytes[cursor] & 0xff) |
                  ((bytes[cursor + 1] & 0xff) << 8) |
                  ((bytes[cursor + 2] & 0xff) << 16) |
                  ((bytes[cursor + 3] & 0xff) << 24);
  console.log(`  Version: ${version} (bytes ${cursor}-${cursor + 3})`);
  cursor += 4;
  
  // Number of inputs (VarInt)
  // For simplicity, we'll assume it's a single byte
  const numInputs = bytes[cursor];
  console.log(`  Number of inputs: ${numInputs} (byte ${cursor})`);
  cursor += 1;
  
  // Parse inputs
  console.log(`  Parsing ${numInputs} inputs:`);
  for (let i = 0; i < numInputs; i++) {
    console.log(`    Input ${i} at position ${cursor}:`);
    
    // Check if we have enough bytes for an outpoint (68 bytes)
    if (cursor + 68 > bytes.length) {
      console.log(`      Not enough bytes for outpoint, only ${bytes.length - cursor} bytes available`);
      break;
    }
    
    // Outpoint: blockHash (32 bytes) + txHash (32 bytes) + index (4 bytes) = 68 bytes
    console.log(`      Outpoint: bytes ${cursor}-${cursor + 67}`);
    cursor += 68;
    
    // Script length (VarInt)
    // For simplicity, we'll assume it's a single byte
    if (cursor >= bytes.length) {
      console.log(`      Not enough bytes for script length`);
      break;
    }
    const scriptLength = bytes[cursor];
    console.log(`      Script length: ${scriptLength} (byte ${cursor})`);
    cursor += 1;
    
    // Script
    if (cursor + scriptLength > bytes.length) {
      console.log(`      Not enough bytes for script, only ${bytes.length - cursor} bytes available`);
      break;
    }
    console.log(`      Script: bytes ${cursor}-${cursor + scriptLength - 1}`);
    cursor += scriptLength;
    
    // Sequence (4 bytes)
    if (cursor + 4 > bytes.length) {
      console.log(`      Not enough bytes for sequence`);
      break;
    }
    console.log(`      Sequence: bytes ${cursor}-${cursor + 3}`);
    cursor += 4;
    
    console.log(`      Input ${i} ends at position ${cursor - 1}`);
  }
  
  console.log(`  Transaction 1 parsing ended at position ${cursor}`);
}

debugTransaction1();
