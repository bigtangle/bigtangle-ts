...
// Test the actual Utils.HEX.decode
...
// Method 2: What we think Utils.HEX.decode does
...
// The real fix: If we know the hex string is missing a leading 0, we should add it
const fixedTip = '0' + tip;
const buffer3 = Buffer.from(fixedTip, 'hex');
console.log('Method 3 (fixed) - Buffer length:', buffer3.length);
console.log('Method 3 (fixed) - Byte at position 160:', buffer3[160].toString(16));

// Check what the actual correct approach should be
// Since the original hex string is missing a leading 0, the correct buffer should be buffer3
// But our parser is reading from buffer2 (Utils.HEX.decode result)
// So the byte that should be at position 160 is actually at position 161 in buffer2
