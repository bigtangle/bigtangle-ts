function debugInputBytes() {
  const tip =
    "";

  // Convert hex string to array of bytes
  const bytes = [];
  for (let i = 0; i < tip.length; i += 2) {
    bytes.push(parseInt(tip.substr(i, 2), 16));
  }
  
  console.log(`Total bytes: ${bytes.length}`);
  
  // The first input starts at cursor 168
  // Let's check how many bytes are available for each input
  const inputStartPositions = [168, 293, 487];
  
  for (let i = 0; i < inputStartPositions.length; i++) {
    const startPos = inputStartPositions[i];
    const bytesAvailable = bytes.length - startPos;
    console.log(`Input ${i}: Start position ${startPos}, bytes available: ${bytesAvailable}`);
    
    if (bytesAvailable >= 68) {
      console.log(`  Enough bytes for outpoint (68 bytes)`);
    } else {
      console.log(`  Not enough bytes for outpoint (68 bytes), missing ${68 - bytesAvailable} bytes`);
    }
  }
  
  // Let's also check what the actual number of inputs should be
  // The number of inputs is read at position 167
  console.log(`\nNumber of inputs byte at position 167: ${bytes[167].toString(16).padStart(2, '0')} (${bytes[167]})`);
}

debugInputBytes();
