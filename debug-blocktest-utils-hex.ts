...
console.log("Analyzing testSerial2 data with Utils.HEX...");
console.log("Hex string length:", tip.length);

// Use Utils.HEX to decode
const bytes = Utils.HEX.decode(tip);
...
const txCountVarInt = readVarInt(buffer, cursor);
...
// Input count
const inputCountVarInt = readVarInt(buffer, cursor);
...
if (cursor < buffer.length) {
    const outputCountVarInt = readVarInt(buffer, cursor);
    ...
    for (let i = 0; i < Math.min(outputCount, 1); i++) {
        ...
        if (cursor < buffer.length) {
            const tokenLenVarInt = readVarInt(buffer, cursor);
            ...
            // Script length (VarInt)
            if (cursor < buffer.length) {
                const scriptLenVarInt2 = readVarInt(buffer, cursor);
                const scriptLen2 = scriptLenVarInt2.value;
                cursor += scriptLenVarInt2.size;
                ...
                if (cursor + scriptLen2 > buffer.length) {
                    ...
                    const script2 = buffer.slice(cursor, cursor + scriptLen2);
                    console.log("Output script bytes:", script2.toString('hex'));
                    cursor += scriptLen2;
                }
            }
        }
...
// Let's also check the last few bytes
const lastBytes = buffer.slice(-20);
console.log("Last 20 bytes:", lastBytes.toString('hex'));

// Now let's try to use the actual Block class to parse this
console.log("\n--- Attempting to use Block class ---");
try {
    const { Block } = require('./src/net/bigtangle/core/Block');
    ...
    if (block.transactions.length > 0) {
        ...
        console.log("First transaction version:", tx.version);
        console.log("First transaction inputs:", tx.inputs.length);
        console.log("First transaction outputs:", tx.outputs.length);
        
        if (tx.outputs.length > 0) {
            console.log("First output value:", tx.outputs[0].value.toString());
        }
    }
} catch (error) {
    console.error("Error parsing with Block class:", error.message);
