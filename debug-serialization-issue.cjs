...
for (let i = 0; i < minLength; i++) {
    if (originalBytes[i] !== reserializedBytes[i]) {
        firstDiff = i;
        console.log(`First difference at byte ${i}: original=0x${originalBytes[i].toString(16).padStart(2, '0')}, reserialized=0x${reserializedBytes[i].toString(16).padStart(2, '0')}`);
        break;
    }
}

if (firstDiff === -1 && originalBytes.length !== reserializedBytes.length) {
    console.log(`No byte differences found, but lengths differ: ${originalBytes.length} vs ${reserializedBytes.length}`);
} else if (firstDiff === -1) {
...
console.log('\n=== End of buffers ===');
console.log('Original buffer end (last 20 bytes):', originalBytes.slice(-20).toString('hex'));
console.log('Reserialized buffer end (last 20 bytes):', reserializedBytes.slice(-20).toString('hex'));
