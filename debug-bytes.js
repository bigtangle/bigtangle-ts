function debugBytes() {
  const tip =
    "";

  // Convert hex string to array of bytes
  const bytes = [];
  for (let i = 0; i < tip.length; i += 2) {
    bytes.push(parseInt(tip.substr(i, 2), 16));
  }
  
  console.log(`Total bytes: ${bytes.length}`);
  
  // Check bytes at positions 159-162
  console.log(`Bytes at positions 159-162:`);
  for (let i = 159; i <= 162; i++) {
    console.log(`  bytes[${i}] = 0x${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
  
  // Calculate the uint32 value
  const result = (bytes[159] & 0xff) |
                 ((bytes[160] & 0xff) << 8) |
                 ((bytes[161] & 0xff) << 16) |
                 ((bytes[162] & 0xff) << 24);
  console.log(`Calculated uint32 value: ${result}`);
  
  // Check bytes around position 159 for context
  console.log(`\nBytes around position 159:`);
  for (let i = 157; i <= 164; i++) {
    console.log(`  bytes[${i}] = 0x${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
}

debugBytes();
