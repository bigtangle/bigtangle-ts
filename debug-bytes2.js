function debugBytes2() {
  const tip =
    "";

  // Convert hex string to array of bytes
  const bytes = [];
  for (let i = 0; i < tip.length; i += 2) {
    bytes.push(parseInt(tip.substr(i, 2), 16));
  }
  
  console.log(`Total bytes: ${bytes.length}`);
  
  // Check bytes at positions 163-170
  console.log(`Bytes at positions 163-170:`);
  for (let i = 163; i <= 170; i++) {
    console.log(`  bytes[${i}] = 0x${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
  
  // Check byte 167 specifically
  console.log(`\nByte 167: bytes[167] = 0x${bytes[167].toString(16).padStart(2, '0')} (${bytes[167]})`);
  
  // Calculate the uint32 value for bytes 163-166 (version)
  const version = (bytes[163] & 0xff) |
                  ((bytes[164] & 0xff) << 8) |
                  ((bytes[165] & 0xff) << 16) |
                  ((bytes[166] & 0xff) << 24);
  console.log(`\nVersion (bytes 163-166): ${version}`);
  
  // Check bytes around position 167 for context
  console.log(`\nBytes around position 167:`);
  for (let i = 165; i <= 172; i++) {
    console.log(`  bytes[${i}] = 0x${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
}

debugBytes2();
