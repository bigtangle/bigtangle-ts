function debugTransaction0() {
  const tip =
    "";

  // Convert hex string to array of bytes
  const bytes = [];
  for (let i = 0; i < tip.length; i += 2) {
    bytes.push(parseInt(tip.substr(i, 2), 16));
  }
  
  console.log(`Total bytes: ${bytes.length}`);
  
  // Check the data around position 153 where transaction 0 starts
  console.log(`Data around position 153 where transaction 0 starts:`);
  for (let i = 151; i <= 160; i++) {
    console.log(`  ${i}: ${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
  
  // Check the data around position 163 where transaction 1 starts
  console.log(`\nData around position 163 where transaction 1 starts:`);
  for (let i = 161; i <= 170; i++) {
    console.log(`  ${i}: ${bytes[i].toString(16).padStart(2, '0')} (${bytes[i]})`);
  }
  
  // Check what the actual version number should be for transaction 0
  // Version is 4 bytes at position 153
  const version0 = (bytes[153] & 0xff) |
                   ((bytes[154] & 0xff) << 8) |
                   ((bytes[155] & 0xff) << 16) |
                   ((bytes[156] & 0xff) << 24);
  console.log(`\nTransaction 0 version (bytes 153-156): ${version0}`);
  
  // Check what the actual version number should be for transaction 1
  // Version is 4 bytes at position 163
  const version1 = (bytes[163] & 0xff) |
                   ((bytes[164] & 0xff) << 8) |
                   ((bytes[165] & 0xff) << 16) |
                   ((bytes[166] & 0xff) << 24);
  console.log(`Transaction 1 version (bytes 163-166): ${version1}`);
}

debugTransaction0();
