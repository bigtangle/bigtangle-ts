...
export class ECKey {
    ...
    public async sign(messageHash: Uint8Array, aesKey?: KeyParameter): Promise<ECDSASignature> {
        if (this.isEncrypted()) {
            ...
            const decryptedPrivKey = await this.keyCrypter.decrypt(this.encryptedPrivateKey, aesKey);
    ...
    public async encrypt(keyCrypter: KeyCrypter, aesKey: KeyParameter): Promise<ECKey> {
        ...
        const encryptedData = await keyCrypter.encrypt(this.getPrivKeyBytes(), aesKey);
    ...
    public async decrypt(keyCrypter: KeyCrypter, aesKey: KeyParameter): Promise<ECKey> {
        ...
        const decryptedPrivKeyBytes = await keyCrypter.decrypt(this.encryptedPrivateKey, aesKey);
    ...
    public async signMessage(message: string): Promise<ECDSASignature> {
        ...
        let offset = 0;
        VarInt.write(messageBuffer.length, { write: (chunk: Buffer) => {
    ...
    public static signedMessageToKey(message: string, signatureBase64: string): ECKey {
        ...
        const sigInstance = secp256k1.Signature.fromCompact(signatureBuffer.slice(1, 65));
    ...
    public static async encryptionIsReversible(originalKey: ECKey, encryptedKey: ECKey, keyCrypter: KeyCrypter, aesKey: KeyParameter): Promise<boolean> {
        try {
